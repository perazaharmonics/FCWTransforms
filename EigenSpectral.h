...
{                                       // ------- ComputeEigenDecomposition -----------
  ...
  for (int i = 0; i < size; i++)
  {
    ...
    // Compute Rayleigh quotient: ? = (b^T * A * b)
    ...
    for (int r = 0; r < size; ++r)
      for (int c = 0; c < size; ++c)
        outer[r][c] = lambda * b[r] * b[c]; // Compute ? * (v * v^T)
...
{                                      // ------- MUSICPhaseEstimation -----------
  ...
  for (int i = 0; i < N; ++i) 
  {
    ...
    // Build the complex steering vector: a(f) = [1, e^{-j2pf}, ..., e^{-j2pf(M-1)}]^T
    for (int k = 0; k < dim; ++k) 
    {
      T phase = -2.0 * M_PI * f * k;   // Compute phase angle
      a[k] = std::complex<T>(std::cos(phase), std::sin(phase)); // e^{-j2pfk}
    }
 ...
 *
 * This function removes 2p ambiguities from a wrapped phase vector by detecting 
 ...
 * does not exceed p radians in magnitude (i.e., no jumps larger than 180 degrees), 
 * by adding or subtracting the appropriate multiple of 2p.
 *
 ...
 * starting from the first phase value (assumed to be within [-p, p)), it iteratively
 * adds or subtracts 2p whenever a jump > p (or < -p) is encountered, effectively 
 * "unwrapping" the phase. This is extended to handle large jumps that may span 
 * multiple 2p intervals, which can occur with under-sampled signals or abrupt 
 * phase shifts in eigenvector data.
 *
 * **Assumptions:** The input vector `phaseData` contains phase angles (in radians) 
 * that are *wrapped* to a principal range, typically [-p, p) or [0, 2p). The function 
 ...
 * 2p discontinuities are removed. Note that true phase changes of ±p (180°) 
 * remain, as they are inherently ambiguous (the algorithm chooses not to alter 
 * differences of exactly p to avoid arbitrary shifts).
 *
 * **Edge Cases:** 
 ...
 * - Large phase discontinuities (> 2p) between samples are handled by applying multiple 
 *   2p corrections iteratively until the jump is within [-p, p]. This means even if the 
 ...
 * - Noisy data: Small oscillations below the p threshold are left untouched. If noise causes 
 *   a jump slightly over the threshold, a 2p correction will be applied (which may or may not 
 *   reflect the true continuous phase – users should pre-filter noise if needed). The algorithm 
 *   uses a threshold of p radians for jumps (with a tiny epsilon margin) to decide unwrapping, 
...
std::vector<T> EigenSpectral<T>::UnwrapPhase(const std::vector<T>& phaseData) const
{
    ...
    // Constants for 2p and p (cast to T to match vector's value type).
    ...
    for (size_t i = 1; i < phaseData.size(); ++i) {
        ...
        // Check for discontinuities greater than p radians.
        if (diff > PI + TOL) {
            // **Positive jump exceeds +p**: we likely wrapped downward (e.g., went from just below +p to just above -p).
            // Compute how many 2p to subtract to reduce the jump. At least one 2p will be subtracted.
            // We use a while-loop to handle cases of multiple wraps (difference much larger than p).
            while (diff > PI + TOL) {
                // Subtract 2p from the current offset to compensate for a wrap-around.
                offset -= TWO_PI;
                diff   -= TWO_PI;
            }
            // After this loop, diff should be <= p (the jump has been reduced by the necessary 2p multiples).
        }
        else if (diff < -PI - TOL) {
            // **Negative jump exceeds -p**: likely wrapped upward (e.g., from just above -p to just below +p).
            while (diff < -PI - TOL) {
                // Add 2p to the offset to compensate for the negative wrap-around.
                offset += TWO_PI;
                diff   += TWO_PI;
            }
            // Now diff should be >= -p.
        }
